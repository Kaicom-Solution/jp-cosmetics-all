...
<div class="rounded shadow-sm">
    <div class="p-3 rounded-top" style="background-color: rgb(119, 82, 125); color:#ffffff">
        <h3 class="fs-5">Edit Skin Type</h3>
    </div>

    <form action="{{ route('skin-type.update', $skinType->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-4 rounded-bottom">
        @csrf

        <div class="row g-4">
            <!-- Logo Upload -->
            <div class="col-md-3 text-center">
                <p class="text-muted">Recommendation Image Size: 645×335 px</p>
            ...
            <div class="col-md-9 mt-10">
                <div class="row g-3">
                    <!-- Name -->
                    <div class="col-md-4">
                        <label class="form-label">Skin Type Name</label>
                        <input required type="text" name="name" id="name" class="form-control"
                               placeholder="Enter Skin Type Name" value="{{ old('name', $skinType->name) }}"/>
                        @error('name')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row g-3 mt-3">
                    ...
                    <div class="col-md-8">
                        ...
                        <textarea name="description" rows="3" class="form-control"
                                  placeholder="Short description...">{{ old('description', $skinType->description) }}</textarea>
                        @error('description')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <input type="hidden" name="slug" value="{{ old('slug', $skinType->slug) }}"/>

                @hasPermission('skin-types.update')
                <div class="row g-3 mt-3">
